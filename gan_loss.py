...
def main():
    ...
    dataset = Data(filename_x = filename_x, filename_y = filename_y, directory = directory, transforms=transforms.Compose([
        transforms.ToTensor()
    ...
    scheduler_g = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optim_G, patience=args.scheduler_patience, verbose = True)
    scheduler_d = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optim_D, patience=args.scheduler_patience, verbose = True)
    ...
    for epoch in range(args.n_epochs):
        ...
        if epoch%args.eval_interval ==0:
            loss_G_val, loss_D_val, mean_psnr_val = eval_epoch(generator, discriminator, test_dataloader, device)
            print(f"Validation on epoch: {epoch}, G: {loss_G_val}, D: {loss_D_val}, PSNR: {mean_psnr_val}")
...
if  __name__ == "__main__":
    ...
    parser.add_argument('--is_big_data', type=bool, default=True,
    ...
    parser.add_argument('--scheduler_patience', type=int, default="10", help="How many epochs of no improvement to consider Plateau")
